...
# Based on real Indian banking loan criteria
# Features: income, credit_score, loan_amount,
#           job_experience, assets_value, existing_loan_amount
# Removed: employment, dependents, own_house, job_type
# -----------------------
data = []

for i in range(3000):

    income               = random.randint(30000, 300000)   # monthly salary Rs 30K - 3L
    credit_score         = random.randint(300, 900)
    loan_amount          = random.randint(100000, 10000000) # 1L to 1 Crore
    job_experience       = random.randint(0, 35)            # 0 to 35 years
    assets_value         = random.randint(50000, 25000000)  # up to 2.5 Crore
    existing_loan_amount = random.randint(0, 5000000)       # existing dues

    # ---- APPROVAL LOGIC (real world rules) ----

    # Rule 1: salary above 3 lakh = auto approve
    if income > 300000:
        approval = 1

    # Rule 2: assets above or equal to 1 crore = auto approve
    elif assets_value >= 10000000:
        approval = 1

    else:
        risk = 0

        # Credit score rules (real bank standards)
        if credit_score < 550:
            risk += 4
        ...
        # Loan to income ratio (real Indian bank standard: max 60x monthly salary)
        ...
        if existing_loan_amount > income * 24:
            risk += 3
        elif existing_loan_amount > income * 12:
            risk += 2
        elif existing_loan_amount > income * 6:
            risk += 1

        # Job experience (banks prefer 2+ years)
        if job_experience < 1:
            risk += 2
        ...
        if assets_value < loan_amount * 0.5:
            risk += 2
        elif assets_value < loan_amount:
            risk += 1

        # Income too low for any loan
        if income < 30000:
            risk += 5
    ...
    data.append([income, credit_score, loan_amount,
                 job_experience, assets_value, existing_loan_amount, approval])
...
while True:
    ...
    if choice == "1":
        ...
        # Job experience - no range shown, validate above 50
        while True:
            try:
        ...
        # Monthly income - range hidden, validated in background
        while True:
            try:
